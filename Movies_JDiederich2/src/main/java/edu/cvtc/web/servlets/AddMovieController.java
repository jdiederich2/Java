...
public class AddMovieController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		final String movieDesc = request.getParameter("movieDesc");
		
		if (Strings.isNullOrEmpty(title) 
				|| Strings.isNullOrEmpty(director)
				|| Strings.isNullOrEmpty(lengthInMinutesString)
				|| Strings.isNullOrEmpty(movieDesc)) {
			...
			try {
				...
				movieDao.insertMovie(new Movie(title, director, lengthInMinutes, movieDesc));
